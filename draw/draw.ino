...
#define STEP_SIZE 10 // ratio of steps to pixels
...
void parseFile(){
  ...
  while((text = br.readLine()) != null){
    String[] coordinateSteps = splitTokens(text,",");
    distX = coordinateSteps[0];
    distY = coordinateSteps[1];
    takeSteps(distX, distY);
  }
  br.close();
}
...
void takeSteps(int distX, int distY){
  ...
  stepsX = abs(distX * STEP_SIZE);
  stepsY = abs(distY * STEP_SIZE);
